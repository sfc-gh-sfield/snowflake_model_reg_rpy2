...
setup script (included in the `r_notebook/` directory):
...
This installs R, `rpy2`, and base R packages via `micromamba`.
...
```r
%%R
# Install from local source (in Workspace)
install.packages("path/to/snowflakeR", repos = NULL, type = "source")

# Or from GitHub (if network access is available)
...
For runnable notebook versions of the examples above, see the companion
Jupyter notebooks in `r_notebook/snowflakeR/`:

- `quickstart.ipynb` -- Connection, queries, DBI, visualization
- `model_registry_demo.ipynb` -- Train, register, predict, manage models
- `feature_store_demo.ipynb` -- Entities, Feature Views, training data
