# Scala in Snowflake Workspace Notebooks: Snowpark, Spark Connect, and %%scala Magic

*A standalone guide to adding Scala support to Snowflake Workspace Notebooks --
Snowpark Scala, Spark Connect, and Python-Scala interoperability*
...
Python and SQL, but what if your team works in Scala? Snowpark has a
first-class Scala SDK, and many data engineering teams prefer Scala for its
type safety, performance, and expressiveness.

In this post I'll show how to add Scala support to Workspace Notebooks --
including Snowpark Scala for direct Snowflake operations and, optionally,
Spark Connect for the full Spark DataFrame API. By the end you'll be able to
write `%%scala` cells in a Workspace Notebook, query Snowflake with
`sfSession.sql(...)`, use the Spark DataFrame API via `spark.sql(...)`, and
pass DataFrames seamlessly between Python and Scala.

The full working prototype -- including the installer, helper module, and
example notebook -- is published as a self-contained repo you can clone into
any Workspace.

## The Approach: JPype as a JVM Bridge

Workspace Notebooks run a Python kernel. There's no JVM or Scala kernel
available. But where there's Python, there's a path to other runtimes. We use
[JPype](https://jpype.readthedocs.io/) to embed a full JVM.
...
serialisation, no network calls. Python and Scala share the same address space.

On top of JPype, we run a Scala REPL (IMain in "Ammonite-lite" mode) and
register a `%%scala` IPython cell magic. The result: you write Scala code in
notebook cells, and it executes as native Scala with full access to Snowpark,
the Scala standard library, and any JARs on the classpath.
...
| Magic | `%%scala` / `%scala` -- custom IPython magic in `scala_helpers.py` |
| REPL | Scala IMain (Ammonite-lite mode) |
| Auth | SPCS OAuth token (auto-detected from the Workspace container) |

## Getting Started

The prototype consists of four files. Upload them to your Workspace Notebook's
working directory:

```
scala_packages.yaml               # Version configuration (Java, Scala, Snowpark)
setup_scala_environment.sh        # Installation script
scala_helpers.py                  # Python helper module (magic, interop, JVM management)
workspace_scala_quickstart.ipynb  # Example notebook with all demos
```
...
This starts the JVM (with the full classpath), initializes the Scala REPL,
registers the `%%scala` and `%scala` magics, and -- if Spark Connect is
enabled -- starts the local gRPC server.
...
That's it. Three steps, and you have a working Scala environment inside a
Snowflake Workspace Notebook.
...
## Python-Scala DataFrame Interop
...
|------|--------|--------|-----------|
| `-i` (auto) | Snowpark Python DF | Snowpark Scala DF | SQL plan transfer |
| `-i` (auto) | PySpark DF | Spark Scala DF | Materialise to table |
| `-i:spark` | Snowpark Python DF | Spark Scala DF | SQL plan → `spark.sql()` |
| `-o` (auto) | Snowpark Scala DF | Snowpark Python DF | SQL plan transfer |
| `-o` (auto) | Spark Scala DF | PySpark DF | Materialise to table |
| `-o:snowpark` | Spark Scala DF | Snowpark Python DF | Materialise to transient table |

Snowpark transfers are zero-copy (only the SQL string crosses the bridge).
Spark Connect transfers materialise through Snowflake tables because the
PySpark and Scala clients run as separate sessions on the gRPC server.
...
| `scala_helpers.py` | Python module -- JVM management, Scala REPL, `%%scala` magic, DataFrame interop, auth |
| `workspace_scala_quickstart.ipynb` | Full working example notebook with all the code from this post |
...
With four files and three setup cells, you get:

- **`%%scala` and `%scala` magics** -- write Scala directly in Workspace
  Notebook cells
- **Snowpark Scala** -- full access to `sfSession.sql(...)`, DataFrame
  transformations, and all Snowpark APIs, authenticated via SPCS OAuth
...
- **DataFrame interop** -- pass Snowpark and PySpark DataFrames between
  Python and Scala with `-i`/`-o` flags, including cross-API transfers

The pattern is straightforward: install a runtime, bridge it into the Python
kernel, and provide a cell magic for a native coding experience. It's not a
supported product feature, but it works today and demonstrates what a
Scala-native Workspace Notebook experience could look like.

---

**Resources:**
- [GitHub: Scala Workspace Prototype](https://github.com/sfc-gh-sfield/snowflake-workspace-scala-prototype)
...
*Tags: Scala, Snowflake, Snowpark, Spark Connect, JPype, Workspace Notebooks*
