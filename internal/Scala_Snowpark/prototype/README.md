...
This calls `sfSession.addDependency(replClassDir)` so Snowpark can locate the
classes generated by the Scala REPL when packaging a UDF JAR.
...
sfSession.sql("SELECT 21 AS val").select(doubleIt(col("val"))).show()
```
